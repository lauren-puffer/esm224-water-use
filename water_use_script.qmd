...
water_use_industry <- rename(county = county_name, 
                             water_use_industry, 
...
gw_cols <- names(water_use_industry)[str_detect(names(water_use_industry), "groundwater")]
...
gw_cols_surf <- names(water_use_industry)[str_detect(names(water_use_industry), "surface_water")]
...
colors <- c("ground" = "#009E73",  # green
            "surface" = "#1f77b4") # blue

# create dataset for Butte county
butte_water <- water_use_long |>
      filter(county == "Butte County")
  
# make a bar graph for butte county water use
butte_bar <- ggplot(butte_water, aes(x = use, y = mgal_day, fill = source)) +
      geom_bar(stat = "identity", position = position_dodge()) +
      scale_fill_manual(values = colors) +
      labs(
        title = "Water Use in Butte County, CA",
        x = "Water Use Type",
...
Now, do the same for plumas.

```{r}
# create dataset for Plumas county
plumas_water <- water_use_long |>
      filter(county == "Plumas County")

# make a bar graph for butte county water use
plumas_bar <- ggplot(plumas_water, aes(x = use, y = mgal_day, fill = source)) +
      geom_bar(stat = "identity", position = position_dodge()) +
      scale_fill_manual(values = colors) +
      labs(
        title = "Water Use in Plumas County, CA",
        x = "Water Use Type",
        y = "Million Gallons per Day",
        fill = "Water Source"
      ) +
...
Repeat the process for Sierra County.

```{r}
# create dataset for Plumas county
sierra_water <- water_use_long |>
      filter(county == "Sierra County")

# make a bar graph for butte county water use
sierra_bar <- ggplot(sierra_water, aes(x = use, y = mgal_day, fill = source)) +
      geom_bar(stat = "identity", position = position_dodge()) +
      scale_fill_manual(values = colors) +
      labs(
        title = "Water Use in Sierra County, CA",
        x = "Water Use Type",
        y = "Million Gallons per Day",
        fill = "Water Source"
      ) +
...
```{r}
# stack bar graphs
combined <- butte_bar/plumas_bar/sierra_bar
print(combined)
```
