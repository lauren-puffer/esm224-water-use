...
scale_fill_manual(
  values = colors,
  labels = c(
    "pct_ground" = "Groundwater",
    "pct_surface" = "Surface Water"
  )
) +
  labs(
    x = "County",
    ...
    fill = "Water Source",
    title = "Percent Water Use by Source per County"
  ) +
  theme_minimal() +
  theme(
    axis.text.x = element_text(size = 12),   # increase x-axis label size
    axis.title.x = element_text(size = 14),  # optional: increase x-axis title size
    legend.text = element_text(size = 12),   # optional: legend text size
